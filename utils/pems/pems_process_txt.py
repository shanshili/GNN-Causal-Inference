...
# 保存为制表符分隔的txt文件（保存在脚本所在目录）
output_path = os.path.join(script_dir, "hour_extracted_data.txt")

if len(combined_data) > 0:
    combined_data.to_csv(output_path, index=False, sep='\t')  # 使用制表符分隔
    tqdm.write(f"\n数据提取完成:")
    tqdm.write(f"- 总共处理了 {len(combined_data)} 行数据")
    tqdm.write(f"- 提取的字段: {', '.join(required_columns)}")
    tqdm.write(f"- 输出文件保存在: {output_path}")
    tqdm.write(f"- 文件格式: 制表符分隔的txt文件（更节省空间）")
