...
"""
# 构建 dataset 文件夹的路径
dataset_dir = os.path.join(current_dir, '..', 'dataset/Intel Lab Data/timestamps_process')
# 确保目标路径存在
...
# 加载数据
position_file = os.path.join(dataset_dir, '30min_filtered_node_locations.csv')
positions_df = pd.read_csv(position_file)

data_path = os.path.join(dataset_dir, '30min_temperature_data.csv')

# 读取数据，跳过第一行(日期戳)，并确保转换为数值
try:
    # 使用pandas读取并跳过第一行
    data = pd.read_csv(data_path, header=None, skiprows=1).astype(float).values

    # 确保数据是 (时间点×节点数) 格式
    if data.shape[1] == 51:  # 如果列数是51 (节点)
        pass  # 已经是正确格式
    elif data.shape[0] == 51:  # 如果行数是51
        data = data.T  # 转置为 (时间×节点)
    else:
        raise ValueError(f"数据形状 {data.shape} 不符合预期")

    print(f"处理后数据形状: {data.shape} (应为(时间点×节点数))")
...
# 转置数据为(时间×节点)格式
if data.shape[0] == 51 and data.shape[1] == 1800:  # 如果是(节点×时间)
    data = data.T  # 转置为(1800×51)
    print(f"转置后数据形状: {data.shape}")


# 定义变量名
var_names = [f'Node_{i}' for i in range(data.shape[0])]  # 51个节点名

# 创建tigramite DataFrame (必须使用numpy数组)
dataframe = pp.DataFrame(data,
                        var_names=var_names,
                        datatime=np.arange(data.shape[0]))  # 添加时间索引
"""
初始化 PCMCI 对象
"""
# 初始化条件独立性检验
...
# 翻转坐标
flipped_pos = {node: (-x, -y) for node, (x, y) in pos.items()}  # 同时翻转x和y
# 绘制节点（使用翻转后的坐标）
...
# 可选：添加节点编号
labels = {i: str(moteid) for i, moteid in enumerate(positions_df['moteid'].values.flatten())}
